      ...
      <el-col :span="8">
        <div class="search-input-wrapper" ref="searchInputWrapper">
          ...
          <transition name="fade-in">
            <div
                v-if="favoriteTagPanelVisible && favoriteTagsForSearch.length"
                class="favorite-tag-panel"
                @mousedown.prevent
                @mouseenter="clearFavoriteHideTimer"
                @mouseleave="scheduleFavoriteHide"
            >
              <div class="favorite-tag-header">
                <span>{{ $t('m.collectTagQuickPick') }}</span>
                <el-button text size="small" @click="favoriteTagPanelVisible = false">{{$t('m.close')}}</el-button>
              </div>
              <el-space wrap size="small" class="favorite-tag-chips">
                <el-tag
                    v-for="tag in favoriteTagsForSearch"
                    :key="tag.id"
                    class="favorite-tag-chip"
                    :style="{ '--tag-color': tag.color || '#409EFF' }"
                    effect="plain"
                    size="small"
                    @click="appendCollectTag(tag, '', $event)"
                    @contextmenu.prevent="appendCollectTag(tag, '-', $event)"
                >
              ...
              <p class="favorite-tag-hint">{{$t('m.collectTagQuickPickHint')}}</p>
            </div>
          </transition>
        </div>
      </el-col>
