  ...
  methods: {
    ...
    handleSortChange(val, bookList) {
      ...
      if (sqlSupportedFilters.includes(val)) {
        // 使用SQL查询
        this.filterBooksBySQL(val, bookList).then(filteredBooks => {
    ...
    isDuplicateGallery(book){
      // 检查该书籍是否有URL，并且该URL在bookList中出现多次
      if (!book.url || book.url === '') return false
      const urlCount = this.bookList.filter(b => b.url === book.url).length
      return urlCount > 1
    },
    // 优化筛选速度：使用SQL数据库查询代替内存过滤
    async filterBooksBySQL(filterType, bookList) {
      ...
      try {
        ...
        switch (filterType) {
          ...
          default:
            ...
            return this.filterBooksByMemory(filterType, bookList)
        }

        // 执行SQL查询
        const sqlResult = await ipcRenderer.invoke('execute-sql-query', {
          query: sqlQuery,
    ...
    filterBooksByMemory(filterType, bookList) {
      ...
      switch (filterType) {
        ...
        case 'nocategory':
          ...
          break
        case 'duplicateGallery':
          filteredBooks = _.filter(bookList, this.isDuplicateGallery)
          break
        default:
